...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/agentlogin", {
        method: "POST",
        headers: {
      ...
      if (res.ok) {
        ...
        sessionStorage.setItem("no", formData.no);

        // Redirect or handle login success (example redirection)
        setTimeout(() => {
          router.push("/agent/dashboard"); // Replace with your actual route
        }, 1000);
      } else {
  ...
  return (
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      ...
      <span className="text-pink-600">Agent</span> Login to Your Account
      </h2>
      {message && (
        <p
